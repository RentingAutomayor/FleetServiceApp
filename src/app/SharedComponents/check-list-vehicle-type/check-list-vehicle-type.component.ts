...
@Component({
  ...
  styleUrls: [
    './check-list-vehicle-type.component.scss',
...
export class CheckListVehicleTypeComponent implements OnInit {
  ...
  setVehicleType(event: any, pVehicleType: VehicleType) {
    if (event.checked) {
      if (
        this.lsVehicleTypeSelected == null &&
        this.lsVehicleTypeSelected == undefined
      ) {
        this.lsVehicleTypeSelected = []
      }
      this.lsVehicleTypeSelected.push(pVehicleType)
    } else {
      const oVehicleTypeTmp = this.lsVehicleTypeSelected.find(
        (item) => item.id == pVehicleType.id
      )
