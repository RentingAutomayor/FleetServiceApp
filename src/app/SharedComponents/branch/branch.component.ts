    ...
    if (this.isToInsert) {
      ...
      if (branchDB.Client_id == 0 || branchDB.Dealer_id == 0) {
        ...
        this.branchService.update(branchDB).subscribe(
          (branchUpdated) => {
            const branchIndex = this.lsBranchs.findIndex(
              (br) => br.id == branch.id
            )
          ...
          (err) => {
            ...
            this.isAwaiting = false
            this.errorTitle =
              'Ocurrió un error intentando Actualizar la sucursal'
            this.errorMessageApi = err.error.Message
          }
        )
      }
    ...
    try {
      if (confirm('¿Está seguro que desea eliminar esta sucursal?')) {
        ...
        const branchToDelete =
          this.completeBranchInformationWithOwner(branchTmp)
        if (branchToDelete.Client_id == 0 || branchToDelete.Dealer_id == 0) {
          ...
          this.branchService.delete(branchToDelete).subscribe(
            ...
            (err) => {
              ...
              this.isAwaiting = false
              this.errorTitle =
                'Ocurrió un error intentando Eliminar la sucursal'
              this.errorMessageApi = err.error.Message
            }
          )
        }
