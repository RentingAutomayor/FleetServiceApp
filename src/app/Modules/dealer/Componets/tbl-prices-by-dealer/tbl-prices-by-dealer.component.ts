...
export class TblPricesByDealerComponent implements OnInit {
  ...
  getListMaintenanceItems(dealerId: number) {
    try {
      ...
      this.maintenanceItemService.getMaintenanceItems(dealerId)
  ...
  getPricesByDealer(dealerId: number) {
    try{
      this.isAwaiting = true;
      this.maintenanceItemService.getPricesByDealer(dealerId)
      .subscribe(itemsByDealer => {
        this.priceByDealer = itemsByDealer;
        this.lsMaintenanceItemsWithPrices = this.priceByDealer.lsMaintenanceItems;
        this.isAwaiting = false;
      });
    }catch (error){
      this.isAwaiting = false;
