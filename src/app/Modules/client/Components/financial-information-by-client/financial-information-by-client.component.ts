...
export class FinancialInformationByClientComponent implements OnInit {
    ...
    try {
      this.transactionService
        ...
        .then((logTrx) => {
          ...
          this.workOrderTransactions = this.lsTransactionByClient.filter(
            (trx) => trx.transaction.movement.id == movementWorkOrder
          )
        })
    } catch (error) {
      console.warn(error)
        ...
        .then((infclient) => {
          ...
          if (this.financialInformationByClient) {
            this.setFinancialInformationByClient(
              this.financialInformationByClient
            )
          }
        })
    } catch (error) {
  ...
  setFinancialInformationByClient(financialInfo: FinancialInformation) {
    const {
      ...
      txtConsumedQuota,
      txtInTransitQuota,
    } = this.frmQuota.controls
    txtApprovedQuota.setValue(
      this.sharedFunctions.formatNumberToString(financialInfo.approvedQuota)
    )
    txtCurrentQuota.setValue(
      this.sharedFunctions.formatNumberToString(financialInfo.currentQuota)
    )
    txtConsumedQuota.setValue(
      this.sharedFunctions.formatNumberToString(financialInfo.consumedQuota)
    )
    txtInTransitQuota.setValue(
      this.sharedFunctions.formatNumberToString(financialInfo.inTransitQuota)
    )
  }
