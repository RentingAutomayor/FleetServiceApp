...
export class PresentationUnitComponent implements OnInit {
  ...
  presentationUnitSelected: PresentationUnit = null
  @Input('presentationUnit')
  set setPresentationUnit(presentation: PresentationUnit) {
    if (presentation) {
      this.presentationUnitSelected = presentation
      if (
        this.presentationUnitSelected !== null &&
        this.presentationUnitSelected !== undefined
      ) {
  ...
  idTypeOfItem: number = 0
  @Input('idTypeOfItem')
  set setIdTypeOfItem(idType: number) {
    if (idType) {
      this.idTypeOfItem = idType
      if (this.idTypeOfItem != 0 && this.idTypeOfItem != undefined) {
  ...
  filterPresentationsByType(idType: number) {
    if (idType) {
      if (this.idTypeOfItem == TypeOfMaintenanceItems.MANO_DE_OBRA) {
        this.lsPresentationUnitFiltered = this.lsPresentationUnit.filter(
          (pres) => pres.id === PresentationUnits.HORAS
        )
      } else {
        this.lsPresentationUnitFiltered = this.lsPresentationUnit.filter(
          (pres) => pres.id !== PresentationUnits.HORAS
  ...
  changePresentationUnit(event: any) {
    this.presentationUnitSelected = this.lsPresentationUnitFiltered.find(
      (pu) => pu.id == event.target.value
    )
    this.changePresentation.emit(this.presentationUnitSelected)
  }
  ...
  setDataInForm(pPresentation: PresentationUnit) {
    ...
    this.frmPresentationUnit.controls.cmbPresentationUnit.setValue(
      idPresentation
    )
  }
