...
export class TransactionReviewComponent implements OnInit {
  ...
  validateCompany() {
    try {
      ...
      if (
        this.companyStorage.type == CompanyType.DEALER ||
        this.companyStorage.type == CompanyType.MAIN_COMPANY
      ) {
  ...
  setDataInForm(transaction: Transaction) {
    const {
      ...
      dealer_branch,
      maintenance_routine,
      trx_valueWithoutTaxes,
      trx_valueTaxes,
      trx_valueWithTaxes,
      trx_valueWithoutDiscount,
      trx_valueDiscount,
      trx_valueWithDiscountWithoutTaxes,
    ...
    if (transaction.headerDetails != null) {
      contract_code.setValue(
        transaction.headerDetails.contract != null
          ? transaction.headerDetails.contract.code
          : ''
      )

      if (transaction.headerDetails.vehicle != null) {
        const licensePlate =
          transaction.headerDetails.vehicle != null
            ? transaction.headerDetails.vehicle.licensePlate
            : ''
        vehicle_licensePlate.setValue(licensePlate)
        const year =
          transaction.headerDetails.vehicle != null
            ? transaction.headerDetails.vehicle.year
            : ''
        vehicle_year.setValue(year)
        const mileage =
          transaction.headerDetails.vehicle.mileage != null
            ? transaction.headerDetails.vehicle.mileage
            : 0
        vehicle_mileage.setValue(
          this.sharedFunction.formatNumberToString(mileage)
        )

        if (transaction.headerDetails.vehicle.vehicleModel != null) {
          const brand =
            transaction.headerDetails.vehicle.vehicleModel.brand.name != null
              ? transaction.headerDetails.vehicle.vehicleModel.brand.name
              : ''
          vehicle_brand.setValue(brand.toUpperCase())
          const type =
            transaction.headerDetails.vehicle.vehicleModel.type.name != null
              ? transaction.headerDetails.vehicle.vehicleModel.type.name
              : ''
          vehicle_type.setValue(type.toUpperCase())
          const model =
            transaction.headerDetails.vehicle.vehicleModel.shortName != null
              ? transaction.headerDetails.vehicle.vehicleModel.shortName
              : ''
      ...
      if (transaction.headerDetails.dealer.name != null) {
        dealer_name.setValue(
          transaction.headerDetails.dealer.name.toUpperCase()
        )
      }

      if (transaction.headerDetails.branch.name != null) {
        dealer_branch.setValue(
          transaction.headerDetails.branch.name.toUpperCase()
        )
      }

      if (transaction.headerDetails.maintenanceRoutine.name != null) {
        maintenance_routine.setValue(
          transaction.headerDetails.maintenanceRoutine.name.toUpperCase()
        )
      }
    }

    trx_valueWithoutDiscount.setValue(
      this.sharedFunction.formatNumberToString(
        transaction.valueWithoutDiscount != null
          ? transaction.valueWithoutDiscount
          : 0
      )
    )
    trx_valueDiscount.setValue(
      this.sharedFunction.formatNumberToString(
        transaction.discountValue != null ? transaction.discountValue : 0
      )
    )
    trx_valueWithDiscountWithoutTaxes.setValue(
      this.sharedFunction.formatNumberToString(
        transaction.valueWithDiscountWithoutTaxes != null
          ? transaction.valueWithDiscountWithoutTaxes
          : 0
      )
    )
    trx_valueTaxes.setValue(
      this.sharedFunction.formatNumberToString(
        transaction.taxesValue != null ? transaction.taxesValue : 0
      )
    )
    trx_valueWithTaxes.setValue(
      this.sharedFunction.formatNumberToString(
        transaction.value != null ? transaction.value : 0
      )
    )
  }
