...
@Component({
  ...
  templateUrl:
    './report-amount-workorders-by-client-or-by-dealer.component.html',
  styleUrls: [
    './report-amount-workorders-by-client-or-by-dealer.component.scss',
  ],
})
export class ReportAmountWorkordersByClientOrByDealerComponent
  ...
  validateDataPosition(data: any) {
    ...
    data.forEach((row) => {
      ...
      if (this.typeOfReport == 'client') {
        ...
        indexDealer = this.chartData.findIndex(
          (dt) => dt.label == row.Concesionario
        )
